...
class SWAG(torch.nn.Module):
    ...
    def transfer_model(self, fake_model):
        # import pdb; pdb.set_trace()
        for (module, name), base_param in zip(self.params, fake_model.parameters()):
            base_param.data = getattr(module, name)
        # fake_model = copy.deepcopy(self.base)
        return fake_model
    ...
    def load_state_dict(self, state_dict, strict=True):
        if not self.no_cov_mat:
            if 'n_models' in state_dict.keys():
